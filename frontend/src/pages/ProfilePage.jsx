...
export default function ProfilePage() {
  ...
  // user.roles may be undefined for legacy users — normalise it
  ...
  const handleSaveDomains = async () => {
    ...
    setDomainError('')
    try {
      const res = await api.put('/profile/domains', { domain_ids: selected })
      setPendingRequests(res.data.pending_requests ?? [])
      setSelected([])
      setDomainSuccess(true)
    } catch (err) {
      setDomainError(err.response?.data?.error || 'Failed to submit domain requests.')
    } finally {
              ...
              {showJoin && (
                <div className="fade-in">
                  <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: 16 }}>
                    Select domains you want to join. Your request will be sent to the admin for approval.
                  </p>
                  ...
                  {fetchingDomains ? (
                    ...
                    <DomainSelector
                      domains={allDomains.filter(d => !(user?.domains ?? []).find(ud => ud.id === d.id))}
                      selected={selected}
                    ...
                    <div style={{ marginTop: 16 }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        {pendingRequests.map(r => (
                          <span key={r.id} className="domain-tag" style={{ opacity: 0.7, background: 'rgba(245,158,11,0.15)', border: '1px solid rgba(245,158,11,0.3)', color: '#fcd34d' }}>
                            ⏳ {r.domain_name}
                          </span>
                        ))}
                      </div>
                  ...
                  {domainSuccess && (
                    <div className="alert alert-success" style={{ marginTop: 12 }}>✅ Request submitted! Awaiting admin approval.</div>
                  )}
                  ...
                  <div style={{ display: 'flex', gap: 12, marginTop: 20 }}>
                    <button
                      ...
                      disabled={savingDomains || selected.length === 0}
                    >
                      {savingDomains ? <><span className="spin">⟳</span> Submitting…</> : <><Save size={15} /> Request to Join</>}
                    </button>
                    <button className="btn btn-ghost" onClick={() => setShowJoin(false)} disabled={savingDomains}>
                      Close
                    </button>
                  </div>
                </div>
